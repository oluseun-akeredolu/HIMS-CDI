...
> **✅ Re-run complete.** These results were generated by an actual end-to-end
> run of `00_load_real_data.py` → `03_freeze.py` against the real 51 official
> train files + 21 official test files, using the fixed `src/csra/calibration.py`
> (floor-collapse and top-edge exclusion bugs both corrected — see `CHANGELOG.md`).
> Total rows loaded matched the manuscript exactly: 7,160,831 train + 1,614,182
> test = 8,775,013 combined. SHA-256 manifest: `frozen_model_real.manifest.json`.

| Model | ECE (pre-fix) | ECE (post-fix, this run) | AUROC (post-fix, this run) |
| :---- | --: | --: | --: |
| **Proposed (BBQ-calibrated)** | 0.0026 | **0.0024** | **0.9796** (95% CI [0.9788, 0.9803]) |
| Uncalibrated Particle Filter | 0.0484 | 0.0484 | 0.9998 |
| Logistic Regression | 0.0114 | 0.0114 | 0.9959 |

The fixes changed the calibrated ECE only marginally on this real dataset
(0.0026 → 0.0024) — the pre-fix headline numbers were not badly biased in
practice, though the underlying bugs were real and could matter more under
distributional shift between TUNE and TEST (see `CHANGELOG.md` item #1 for a
constructed example where the pre-fix bug reported 0% risk for flows that
were actually 45% likely to be attacks). The uncalibrated-PF and Logistic
Regression baselines are effectively unchanged, as expected — neither one
exercises the parts of `calibration.py` that were fixed in a way that this
particular data distribution happened to trigger.
